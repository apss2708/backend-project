...
def process_transaction_csv(job_id_str: str, file_path: str):
    ...
    # Ensure database tables exist on execution (ensures worker container can self-initialize tables)
    ...
    try:
        ...
        try:
            # Passes SQLAlchemy Session for historical median lookup
            detect_anomalies(db, cleaned_records)
        ...
        for idx, r in enumerate(cleaned_records):
            # If category is not in allowed categories (case-sensitive check)
            if r["category"] not in ALLOWED_CATEGORIES:
                r["temp_index"] = idx
                rows_to_classify.append(r)
                
        if rows_to_classify:
            logger.info(f"[Job {job_id_str}] Classified row count: {len(rows_to_classify)}. Processing in batches of 15.")
            batch_size = 15
            for i in range(0, len(rows_to_classify), batch_size):
                batch = rows_to_classify[i:i+batch_size]
                classify_categories_batch(batch)
        ...
        # Save transactions to DB (before final metrics and summaries are computed/retrieved)
        logger.info(f"[Job {job_id_str}] Saving {clean_count} transactions to database...")
        db_transactions = []
        ...
        db.flush()  # Push transactions to DB session to resolve IDs but don't commit yet
        ...
        try:
            ...
            # Summary generation fails are non-fatal to the pipeline
            # Create a simple fallback summary record
        ...
        db.rollback()
        # Fallback to updating job to failed
        try:
            job = db.query(Job).filter(Job.id == job_id).first()
            if job:
                job.status = "failed"
